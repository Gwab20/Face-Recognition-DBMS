--  STUDENT RECORD SYSTEM — FINALIZED SCHEMA
--  Face-ID based attendance + student record management

-- USERS
-- Central auth table. role: 'teacher' | 'student'
-- face_encoding stores the YOLO/face-ID vector (text/base64)
CREATE TABLE users (
...
-- ACCESS LOG
-- Audit trail: who did what and to which record
...
-- TEACHERS
-- One teacher profile per user (where role = 'teacher')
...
-- COURSES
-- A course belongs to one teacher. Soft-delete via is_active.
...
-- STUDENTS
-- One student profile per user (where role = 'student')
-- Students can view their own records but cannot edit them.
...
CREATE TABLE attendance (
    ...
    check_in_time   TIME,
    remarks         TEXT,
    UNIQUE (student_id, course_id, attendance_date)  -- one record per student/course/day
);

-- FACE RECOGNITION LOG
-- Every time YOLO attempts a face scan, log it here.
-- Links back to the attendance record it created (if success).
...
-- ASSESSMENTS
-- A specific assessment (e.g. "Quiz 1") for a course
...
-- Only teachers (via updated_by) should INSERT/UPDATE here.
CREATE TABLE marks (
    ...
    updated_at    TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    UNIQUE (student_id, assessment_id)  -- one score per student per assessment
);
...
--  MOCK DATA — Demo seed
--  Teacher: Ali Akber
--  Students: Muhammad Awab, Abdul Rehman, Shariq Usman

-- USERS
-- Passwords are bcrypt hashes of 'password123' (swap before prod)
INSERT INTO users (user_id, email, password_hash, role) VALUES
(1, 'ali.akber@giki.edu.pk',      '$2b$12$KIXbV1v1v2e5e5e5e5e5eOQKIXbV1v1v2e5e5e5e5e5eOQplaceholdr', 'teacher'),
(2, '2024358@giki.edu.pk',  '$2b$12$KIXbV1v1v2e5e5e5e5e5eOQKIXbV1v1v2e5e5e5e5e5eOQplaceholdr', 'student'),
(3, '2024027@giki.edu.pk',   '$2b$12$KIXbV1v1v2e5e5e5e5e5eOQKIXbV1v1v2e5e5e5e5e5eOQplaceholdr', 'student'),
(4, '2024465@giki.edu.pk',   '$2b$12$KIXbV1v1v2e5e5e5e5e5eOQKIXbV1v1v2e5e5e5e5e5eOQplaceholdr', 'student');
...
-- COURSES
-- Two courses taught by Ali Akber for a realistic demo
...
-- ENROLLMENTS
-- All three students enrolled in both courses
...
-- ASSESSMENTS
-- CS301 — AI: Quiz 1, Assignment 1, Midterm
-- CS201 — OOP: Quiz 1, Assignment 1
...
-- MARKS
-- Realistic scores — not everyone aces everything
-- updated_by = 1 (Ali Akber's user_id)
...
-- ATTENDANCE
-- A week's worth of attendance for both courses
-- marked_by = 1 (Ali Akber), mix of face_id and manual
INSERT INTO attendance (student_id, course_id, attendance_date, status, marked_by, marked_via, check_in_time) VALUES
-- CS232 DBMS — Week 1 (Mon/Wed/Fri)
